...
namespace QuantCpp::Trading
{
    ...
    auto MarketDataConsumer::startSnapshotSync() -> void
    {
        ...
        ASSERT(snapshot_mcast_socket_.init(snapshot_ip_, iface_, snapshot_port_, true) >= 0,
               "Failed to initialize snapshot multicast socket. error: " +
                   std::to_string(errno));
    ...
    auto MarketDataConsumer::checkSnapshotSync() -> void
    {
        ...
        {
            logger_.log("%:% %() % Returning because have not seen a SNAPSHOT_START message yet.\n",
                        __FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_));
            snapshot_queued_msgs_.clear();
            return;
        }
        ...
        for (auto &snapshot_itr : snapshot_queued_msgs_)
        {
            ...
            if (snapshot_itr.first != next_snapshot_seq)
            {
                ...
                logger_.log("%:% %() % Deteced gap in snapshot stream expected: % found:% %.\n",
                            __FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_),
            ...
            {
                logger_.log("%:% %() % Detected incomplete snapshot stream, clearing snapshot_queued_msgs_.\n",
                            __FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_));
                snapshot_queued_msgs_.clear();
                return;
            }
            ...
            {
                logger_.log("%:% %() % Detected incomplete snapshot stream, last message was not SNAPSHOT_END: \n",
                            __FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_));
                return;
            }
            ...
            {
                logger_.log("%:% %() % Detected incomplete incremental stream, clearing incremental_queued_msgs_.\n",
                            __FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_));
                incremental_queued_msgs_.clear();
                return;
            }
            ...
            {
                auto next_write = incoming_md_updates_->getNextWriteTo();
                *next_write = event;
                incoming_md_updates_->updateWriteIndex();
            }

            logger_.log("%:% %() % Processed snapshot with % events and % incremental events.\n",
                        __FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_),
    ...
    auto MarketDataConsumer::recvCallback(McastSocket *socket) noexcept -> void
    {
        ...
        if (UNLIKELY(is_snapshot && !in_recovery_))
        {
            ...
            logger_.log("%:% %() % Received snapshot message while not in recovery mode. Ignoring.\n",
                        __FILE__, __LINE__, __FUNCTION__, getCurrentTimeStr(&time_str_));
            return;
        }

        if (socket->next_recv_valid_index_ >= sizeof(Exchange::MDPMarketUpdate))
        {
            size_t i = 0;
            for (; i + sizeof(Exchange::MDPMarketUpdate) <= socket->next_recv_valid_index_; i += sizeof(Exchange::MDPMarketUpdate))
            {
