...
namespace QuantCpp::Trading
{
    OrderGateway::OrderGateway(ClientId client_id, Exchange::ClientRequestLFQueue *client_requests,
                               ...
                               std::string ip, const std::string &iface, int port) : client_id_(client_id),
                                                                                     ip_(ip),
                                                                                     iface_(iface),
                                                                                     port_(port),
    ...
    {
        run_ = start;
        ASSERT(tcp_socket_.connect(ip_, iface_,
                                   port_, false) >= 0,
               "Unable to connect to ip:" + ip_ +
                   " port:" + std::to_string(port_) +
                   " on iface:" + iface_ +
                   " error:" + std::string(std::strerror(errno)));

        ASSERT(createAndStartThread(-1, "Trading/OrderGateway",
                                    [this]()
                                    { run(); }) != nullptr,
                                    ...
                                    Nanos rx_time) noexcept -> void
    {
        ...
        if (socket->next_rcv_valid_index_ >= sizeof(Exchange::OMClientResponse))
        {
                 ...
                 i += sizeof(Exchange::OMClientResponse))
            {
                ...
                auto next_write = incoming_responses_->getNextWriteTo();
