...
namespace QuantCpp::Exchange
{
    ...
    {
    public:
        FIFOSequencer(ClientRequestLFQueue *client_request,
                      Logger *logger) : incoming_requests_(client_request), logger_(logger) {}

        ~FIFOSequencer() = default;

        auto addClientRequest(Nanos rx_time,
                              const MEClientRequest &request)
        {
        ...
        auto sequenceAndPublish()
        {
            ...
            for (size_t i = 0; i < pending_size_; ++i)
            {
                ...
                logger_->log("%:% %() % Writing RX:% Req:% to FIFO.\n",
                             __FILE__, __LINE__, __FUNCTION__,
                             getCurrentTimeStr(&time_str_), client_request.recv_time_,
                             client_request.request_.toString());

                auto next_write = incoming_requests_->getNextWriteTo();
