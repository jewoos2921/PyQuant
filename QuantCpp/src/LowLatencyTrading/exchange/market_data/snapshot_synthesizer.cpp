...
namespace QuantCpp::Exchange
{
    ...
    {
        run_ = true;
        ASSERT(createAndStartThread(-1, "Exchange/SnapshotSynthesizer", [this]()
                                    { run(); }) != nullptr,
               "Failed to start snapshot synthesizer thread.");
    }
    ...
    auto SnapshotSynthesizer::addToSnapshot(const MDPMarketUpdate *market_update)
    {
        ...
        case MarketUpdateType::ADD:
        {
            ...
            ASSERT(order == nullptr, "Received: " + me_market_update.toString() + ", but already exists" +
        ...
        {
            auto order = orders->at(me_market_update.order_id_);
            ASSERT(order != nullptr, "Received: " + me_market_update.toString() + ", but does not exist");
