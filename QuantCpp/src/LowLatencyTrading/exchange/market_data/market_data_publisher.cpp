...
namespace QuantCpp::Exchange
{
    MarketDataPublisher::MarketDataPublisher(MEMarketUpdateLFQueue *market_updates,
        ...
        : outgoing_md_updates_(market_updates),
          ...
          run_(false),
          logger_("exchange_market_data_publisher"), incremental_socket_(logger_)
    {
    ...
    {
        run_ = true;
        ASSERT(createAndStartThread(-1, "Exchange/MarketDataPublisher", [this]()
                                    { run(); }) != nullptr,
               "Failed to start Market data thread.");
    ...
    auto MarketDataPublisher::run() noexcept -> void
    {
        ...
        while (run_)
        {
                 ...
                 market_update = outgoing_md_updates_->getNextToRead())
            {
                ...
                auto next_write = snapshot_md_updates_.getNextWriteTo();
