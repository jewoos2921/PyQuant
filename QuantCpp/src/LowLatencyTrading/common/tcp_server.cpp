...
namespace QuantCpp::Common
{
    ...
    auto TCPServer::addToEpollList(TCPSocket *socket)
    {
        ...
        return !epoll_ctl(epoll_fd_,
                          EPOLL_CTL_ADD, socket->socket_fd_, &ev);
    }
    ...
    auto TCPServer::listen(const std::string &iface, int port) -> void
    {
        ...
        ASSERT(epoll_fd_ >= 0,
               "epoll_create() failed error:" + std::string(strerror(errno)));
        ...
        ASSERT(addToEpollList(&listener_socket_),
               "Failed to add listener socket to epoll list. error:" + std::string(std::strerror(errno)));
    }
    ...
    auto TCPServer::sendAndRecv() noexcept -> void
    {
        ...
        std::for_each(send_sockets_.begin(),
                      send_sockets_.end(), [](auto socket)
                      { socket->sendAndRecv(); });
    }
}